...
const AZURE_OPENAI_DEPLOYMENT =
  process.env.NEXT_PUBLIC_AZURE_OPENAI_DEPLOYMENT || "gpt-4.0";
...
export const getAIResponse = async (
  ...
  try {
    ...
    const systemPrompts = {
      en: `You are an HR professional interviewing a candidate for the ${
        options.position || "professional"
      } position. 
           Ask professional questions according to interview standards.
           Focus on skills: ${
             options.skills || "communication, problem solving, teamwork"
           }.
      ...
      vi: `Bạn là một chuyên gia tuyển dụng (HR) đang phỏng vấn ứng viên cho vị trí ${
        options.position || "chuyên viên"
      }. 
           Hãy đặt các câu hỏi chuyên nghiệp, theo đúng tiêu chuẩn phỏng vấn. 
           Tập trung vào kỹ năng: ${
             options.skills || "giao tiếp, giải quyết vấn đề, làm việc nhóm"
           }.
    ...
    // Chọn prompt theo ngôn ngữ và kết hợp với instruction tùy chỉnh nếu có
    let systemPrompt = systemPrompts[options.language || "vi"];
...
const getMockResponse = (
  ...
  const questionsMap = {
    en: {
      ...
      greetings:
        "Hello! Nice to meet you. Could you tell me about your work experience?",
      ...
      salary:
        "The salary will be considered based on your experience and skills.",
      ending:
        "Thank you for participating in today's interview. We appreciate your time and interest. We will contact you in the next few days with the results.",
    },
    vi: {
      questions: [
      ...
      greetings:
        "Xin chào! Rất vui được gặp bạn. Bạn có thể giới thiệu thêm về kinh nghiệm làm việc của mình không?",
      ...
      salary:
        "Dạ, mức lương sẽ được xem xét dựa trên kinh nghiệm và kỹ năng của bạn.",
      ending:
        "Cảm ơn bạn đã tham gia buổi phỏng vấn hôm nay. Chúng tôi đánh giá cao thời gian và sự quan tâm của bạn. Chúng tôi sẽ liên hệ với bạn trong vài ngày tới để thông báo kết quả nhé.",
    },
  };

  const currentLang = questionsMap[language];
  const messageCount = conversationHistory.filter(
    (msg) => !msg.isTyping && !msg.isError
  ).length;
  ...
  if (
    ...
    lowerMsg.includes("kinh nghiệm") ||
    lowerMsg.includes("project") ||
    lowerMsg.includes("dự án") ||
    messageCount < 2
  ) {
